export const APP_URL = '';
export const ANNOUNCEMENT_NOTIFICATION = 1;
export const QUIZ_NOTIFICATION = 2;
export const SYSTEM_NOTIFICATION = 3;
export const CREATED_ANNOUNCEMENT_ID = 1;
export const DEBOUNCE_TIME = 300;
export const USER_ROLE_ID = 1;
export const MODER_ROLE_ID = 2;
export const ADMIN_ROLE_ID = 3;
export const ANNOUNCEMENT_CREATED = 1;
export const VOTE_MODE = 7;
export const ANNOUNCEMENT_IMPORTANT = 3;
export const ANNOUNCEMENT_APPROVED = 2;
export const QUIZ_PLAY_ACTIVITY = 1;
export const QUIZ_CREATE_ACTIVITY = 2;
export const ACHIEVEMENT_RECEIVE_ACTIVITY = 3;
export const ANNOUNCEMENT_CREATE_ACTIVITY = 4;
export const QUIZ_LIKE_ACTIVITY = 5;
export const USER_STATUS_ACTIVE = 2;
export const USER_STATUS_DEACTIVE = 3;
export const USER_STATUS_REJECTED = 6;
export const USER_POINTS = 10;
export const OPTIONAL_QUESTION = 3;
export const SEQUENCE_QUESTION = 4;
export const TRUE_FALSE_QUESTION = 1;
export const INPUT_QUESTION = 2;
export const ANNOUNCEMENT_DEFAULT_IMAGE = 'https://cdn.pixabay.com/photo/2020/04/09/12/28/dog-5021242_1280.jpg';
export const USER_DEFAULT_IMAGE = 'https://img.icons8.com/plasticine/100/000000/user-male-circle.png';
export const ELECTION_DEFAULT_IMAGE = '../../assets/Gerb.svg.png';
export const CANDIDATE_STATUS_ID = 7;
export const CANDIDATE_ROLE_ID = '5';